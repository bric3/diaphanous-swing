 ...
 * </ul>
 */
public final class WindowRevealController {
    private static final boolean IS_MAC = System.getProperty("os.name", "").contains("Mac");
    private static final RevealControllerDelegate DELEGATE = IS_MAC
        ? new MacosRevealControllerDelegate()
        : new DefaultRevealControllerDelegate();

    private WindowRevealController() {}
    ...
    public static void show(Window window, Runnable onRevealed) {
        DELEGATE.show(window, onRevealed);
    }

    private interface RevealControllerDelegate {
        void show(Window window, Runnable onRevealed);
    }

    private static final class DefaultRevealControllerDelegate implements RevealControllerDelegate {
        @Override
        public void show(Window window, Runnable onRevealed) {
            window.setVisible(true);
            runCallback(onRevealed);
        }
    }

    private static final class MacosRevealControllerDelegate implements RevealControllerDelegate {
        ...
        private void revealOnFirstPaint(Window window, Runnable onRevealed) {
        ...
        private void reveal(Window window) {
            try {
                if (WindowBackdrop.isSupported()) {
